#!/usr/bin/env python3
"""Run one exact, content-free pre-1.0 qualification case for client-typescript."""
...
def _case_command(value: object, label: str) -> list[str]:
    ...
    exact_node_test = (
        ...
        and command[5] == f"--test-name-pattern=^{re.escape(str(assertion))}$"
        and command[6].startswith("tests/")
        and command[6].endswith(".test.ts")
    )
    if (
        ...
        or not exact_node_test
    ):
        raise RuntimeError(f"qualification case is not an exact Node assertion: {label}")
    return command
...
def description() -> dict:
    return {
        ...
        "semantic_binding": {
            ...
            "exact_assertion_result_required": True,
...
def exact_tap_assertion_passed(output: str, assertion: str) -> bool:
    return (
        f"# Subtest: {assertion}" in output
        and re.search(rf"(?m)^\s*ok \d+ - {re.escape(assertion)}\s*$", output) is not None
        and re.search(r"(?m)^# tests 1\s*$", output) is not None
        and re.search(r"(?m)^# pass 1\s*$", output) is not None
        and re.search(r"(?m)^# fail 0\s*$", output) is not None
        and re.search(r"(?m)^# skipped 0\s*$", output) is not None
    )
