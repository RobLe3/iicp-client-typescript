...
describe("#409 buildCapabilities", () => {
  ...
  it("chat-only yields a single capability (back-compat)", () => {
  ...
  it("empty models yields one default-intent capability", () => {
    const caps = buildCapabilities([], CHAT, 1024);
    assert.deepEqual(caps, [{ intent: CHAT, models: [], max_tokens: 1024 }]);
  });
});
